...
class SnakeGame:
    ...
    def play_step(self):
        ...
        if self.head == self.food:
            self.score += 1
            self.speed += 1  # Increase speed on eating food
            self._place_food()
        else:
            self.snake.pop()
    ...
    def game_over(self):
        over_font = pygame.font.Font('arial.ttf', 50)
        over_text = over_font.render('Game Over', True, RED)
        self.display.blit(over_text, [self.w // 4, self.h // 3])
        score_text = font.render(f'Final Score: {self.score}', True, WHITE)
        self.display.blit(score_text, [self.w // 4, self.h // 2])
        restart_text = font.render('Press R to Restart or ESC to Exit', True, WHITE)
        self.display.blit(restart_text, [self.w // 4, self.h // 1.5])
        pygame.display.flip()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game_over:
            game.game_over()  # Show game over screen and wait for action
